<?php 
    ...
    $date1 = $_POST['id'];
    $date2 = $_POST['id2'];
    $sql = $conn->prepare("SELECT * FROM list WHERE date BETWEEN '$date1' AND date  '$date2' ");
    $sql->execute();
    $show = $sql->fetchAll(PDO::FETCH_ASSOC);
    $number = 1;
    $sum = $conn->prepare("SELECT sum(price) FROM list WHERE date BETWEEN '$date1' AND date '$date2'");
    $sum->execute();
